...
function takeNextVariant(levelIndex) {
  ...
  const hasContextualPowerIntro = Boolean(
    config?.crystalIntro ||
    config?.tornadoIntro ||
    config?.hammerIntro ||
    config?.fishingIntro ||
  ...
  if (state.current < 0 && hasContextualPowerIntro) {
    state.current = 0;
...
function activateCrystalPower() {
  ...
  if (!path.length) {
    crystalCandidatePath = [];
    crystalCandidateIncomplete = false;
    setMessage("No safe route to the next objective is available.", true);
    return;
  }
...
function activateHammerPower() {
  ...
  if (!getHammerTargets().length) {
    setMessage("There is no breakable wall, rock, or fallen bottle beside the mouse.", true);
    return;
  }
...
function ensurePregeneratedCarStartsMobile(result, config, variantIndex) {
  ...
  if (!cheeseOnlyPuzzle && carHasLegalMove(result.grid, result.car, blockedKeys)) {
    return result;
  }
  if (
    cheeseOnlyPuzzle &&
    carPlacementOffersFairStart(result, result.grid, result.start, result.car) &&
    carPlacementKeepsSolutionSafe(result, result.grid, result.start, result.car)
  ) {
    return result;
  }
  ...
  for (const candidate of candidates) {
    const solution = cheeseOnlyPuzzle
      ...
      : null;
    if (solution || carPlacementKeepsSolutionSafe(result, result.grid, result.start, candidate)) {
...
const GARDEN_THEME_LAST_LEVEL = 42;
...
function tutorialCropWallGeometry(segment, corners, width, columns, rows) {
  ...
  const gardenScale = gardenThemeActive() ? 1.08 : 1;
  return {
...
function drawTutorialGameCrop(ctx, left, top, width, height, columns = 5, rows = 3) {
  ...
  const corners = {
    tl: { x: left - width * 0.035, y: top - height * 0.035 },
    tr: { x: left + width * 1.035, y: top - height * 0.035 },
    br: { x: left + width * 1.075, y: top + height * 1.055 },
    bl: { x: left - width * 0.075, y: top + height * 1.055 },
  };
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let col = 0; col < columns; col += 1) {
      ...
      ctx.lineWidth = 1;
      ctx.stroke();
      drawGardenTileDetail(ctx, points, row + 70, col + 90);
    }
  }

  const wallSegments = rows >= 5
    ? [
        { orientation: "horizontal", line: 0.82, start: 0, length: 1.16 },
        { orientation: "vertical", line: 1.16, start: 0, length: 0.82 },
        { orientation: "horizontal", line: 1.72, start: 3.72, length: 1.28 },
        { orientation: "vertical", line: 3.72, start: 0.9, length: 0.82 },
        { orientation: "horizontal", line: 3.08, start: 0, length: 1.2 },
        { orientation: "vertical", line: 1.2, start: 3.08, length: 1.1 },
        { orientation: "horizontal", line: 4.18, start: 3.8, length: 1.2 },
        { orientation: "vertical", line: 3.8, start: 3.08, length: 1.1 },
      ]
    : [
        { orientation: "horizontal", line: 0.72, start: 0, length: 1.12 },
        { orientation: "vertical", line: 1.12, start: 0, length: 0.72 },
        { orientation: "horizontal", line: 0.72, start: 3.88, length: 1.12 },
        { orientation: "vertical", line: 3.88, start: 0, length: 0.72 },
        { orientation: "horizontal", line: 2.32, start: 0, length: 1.08 },
        { orientation: "vertical", line: 1.08, start: 2.32, length: 0.68 },
        { orientation: "horizontal", line: 2.32, start: 3.92, length: 1.08 },
        { orientation: "vertical", line: 3.92, start: 2.32, length: 0.68 },
      ];
  const walls = wallSegments
    .map((segment) =>
      tutorialCropWallGeometry(segment, corners, width, columns, rows),
    )
  ...
  ctx.save();
  ctx.strokeStyle = gardenThemeActive()
    ? "rgba(87, 111, 51, 0.58)"
    : "rgba(92, 111, 99, 0.54)";
  ...
  const centerTop = point(2, 0);
  const centerBottom = point(2, 2);
  return {
    corners,
    point,
    polygon,
    cellWidth: width / columns,
    cellHeight: distance(centerTop, centerBottom) / 2,
  };
}
...
function drawMechanicTutorialOverlay(ctx, width, height) {
  ...
  const squareTutorial = ["crow", "cloud", "water"].includes(mechanicTutorial.kind);
  ...
  ctx.save();
  ctx.beginPath();
  ctx.roundRect(floorLeft, floorTop, floorWidth, floorHeight, 8);
  ctx.clip();
  ...
  if (mechanicTutorial.kind === "cheese") {
    ...
    if (spill > 0) {
      drawMilkPuddle(ctx, point(3.25, 1), cell, {
        opacity: 0.94,
        scale: spill,
        angle: 0,
        depletion: drink * 0.94,
      });
    }
    ...
    drawInteriorWallSet(ctx, [{
      p1: hinge,
      p2: {
        x: hinge.x + Math.cos(hingeAngle) * cell * 1.15,
        y: hinge.y + Math.sin(hingeAngle) * cell * 1.15,
      },
      lineWidth: cell * 0.18,
      height: cell * 0.14,
      depth: 0.5,
    ...
    const carCenter = { x: carStart.x + cell * 1.48 * chase, y: carStart.y };
    const mouseCenter = { x: mouseStart.x + cell * 0.46 * chase, y: mouseStart.y };
    ...
    if (eaten < 4) {
      drawTutorialSprite(ctx, pieSprite, pieCenter, cell * 2.05, 0, 1);
    }
    ...
    for (let index = 0; index < eaten && eaten < 4; index += 1) {
      ctx.fillRect(quarters[index][0], quarters[index][1], half, half);
      ctx.save();
      ctx.fillStyle = "rgba(181, 138, 82, 0.38)";
      ctx.beginPath();
      ctx.arc(
        quarters[index][0] + half * 0.5,
        quarters[index][1] + half * 0.5,
        cell * 0.07,
        0,
        Math.PI * 2,
      );
    ...
    const approachCells = [
      { x: pieCenter.x + cell * 1.28, y: pieCenter.y - cell * 0.5 },
      { x: pieCenter.x + cell * 0.5, y: pieCenter.y + cell * 1.05 },
      { x: pieCenter.x - cell * 1.28, y: pieCenter.y + cell * 0.5 },
      { x: pieCenter.x - cell * 0.5, y: pieCenter.y - cell * 1.05 },
    ];
    ...
    const order = [
      [0,2],[1,2],[2,2],[2,1],[2,3],[3,1],[3,3],
      [4,1],[4,3],[3,2],[4,2],[1,1],[1,3],[0,1],[0,3],
    ];
    ...
    ctx.beginPath();
    ctx.moveTo(entry.x + cell * 0.42, entry.y + cell * 0.16);
    ctx.quadraticCurveTo(
      (entry.x + destination.x) / 2,
      entry.y + cell * (0.45 + undergroundProgress * 0.18),
      destination.x - cell * 0.42,
      destination.y + cell * 0.16,
    );
    ctx.stroke();
    ctx.restore();
...
function createBoardGeometry(width, height) {
  ...
  const frameWidth = gardenThemeActive()
...
function drawMazeBoard(ctx, width, height, geometry = createBoardGeometry(width, height)) {
  ...
  if (!shapedBoard) drawOuterFrame(ctx, outer, inner, frameWidth, frameDepth);
...
function drawBoardShadow(ctx, outer, depth) {
  const palette = activeMazePalette();
  ctx.save();
  ctx.filter = gardenThemeActive() ? "blur(22px)" : "blur(18px)";
...
function drawFloor(ctx, inner, width) {
  ...
  for (let row = 0; row < ROWS; row += 1) {
    for (let col = 0; col < COLS; col += 1) {
      ...
      ctx.lineWidth = 1;
      ctx.stroke();
      drawGardenTileDetail(ctx, points, row, col);
    }
  }
...
function segmentGeometry(segment, corners, width) {
  ...
  const gardenWidthScale = gardenThemeActive() ? 1.22 : 1;
  const gardenHeightScale = gardenThemeActive() ? 0.94 : 1;
  return {
    p1: project(corners, u1, v1),
    p2: project(corners, u2, v2),
...
function activeMazePalette() {
  return gardenThemeActive() ? GARDEN_MAZE_PALETTE : PANTRY_MAZE_PALETTE;
}
...
function gardenWallTopGradient(ctx, wall, palette, frameStyle = false) {
...
function drawInteriorWallSet(ctx, walls, style = "interior") {
  const palette = activeMazePalette();
  const frameStyle = gardenThemeActive() && style === "frame";
  ...
  ctx.shadowBlur = gardenThemeActive() ? (frameStyle ? 13 : 9) : 10;
  ctx.shadowOffsetY = gardenThemeActive() ? (frameStyle ? 10 : 7) : 8;
  ...
  for (const wall of walls) {
    const topFill = gardenThemeActive()
      ? gardenWallTopGradient(ctx, wall, palette, frameStyle)
      : top;
...
function drawOuterFrame(ctx, outer, inner, lineWidth, depth) {
  const palette = activeMazePalette();
  ...
  const drawFrame = () => {
    ...
    drawPath(0, lineWidth + (gardenThemeActive() ? 2.4 : 4), palette.frameOutline);
    if (gardenThemeActive()) {
    ...
    drawPath(
      -lineWidth * 0.2,
      Math.max(1.5, lineWidth * (gardenThemeActive() ? 0.13 : 0.2)),
      palette.frameHighlight,
    );
    if (gardenThemeActive()) {
      ctx.beginPath();
