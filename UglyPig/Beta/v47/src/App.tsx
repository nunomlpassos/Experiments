...
interface RewardedAdsPlugin {
  prepareRewardedAd: () => Promise<{ loaded: boolean; loading: boolean }>;
...
const RUN_INTRO_DURATION_MS = 2400;
...
const GAMEPLAY_WIDTH = 430;
const BUTTON_PRESS_FEEDBACK_MS = 140;
const SHOW_DEBUG_LABELS = false;
const DIGIT_IMAGES = [d0, d1, d2, d3, d4, d5, d6, d7, d8, d9];

const showRewardedContinueAd = async () => {
  if (!Capacitor.isNativePlatform()) return true;

  try {
    const result = await RewardedAds.showRewardedAd();
    return result.rewarded === true;
  } catch (error) {
    console.error("Could not complete rewarded continue ad", error);
    return false;
  }
};

const prepareRewardedContinueAd = () => {
  if (!Capacitor.isNativePlatform()) return;
  RewardedAds.prepareRewardedAd().catch((error) => {
    console.error("Could not prepare rewarded continue ad", error);
  });
};

function PressableButtonStyles() {
  return (
    <style>{`
      .pressable-button {
      ...
      .pressable-button-large {
        transform: scale(1.02);
      }
      .pressable-button:active,
      .pressable-button.is-pressed {
        transform: scale(.9);
        filter: brightness(.78) saturate(.96);
      }
      .pressable-button-large:active,
      .pressable-button-large.is-pressed {
        transform: scale(.92);
      }
...
const getGameplayScale = (viewportWidth: number) => viewportWidth / GAMEPLAY_WIDTH;

const getGameplaySize = (viewportSize: { width: number; height: number }) => ({
  width: GAMEPLAY_WIDTH,
  height: Math.max(500, viewportSize.height / getGameplayScale(viewportSize.width)),
});
...
function IntroCornerPost({ left, top }: { left: number; top: number }) {
  return (
    <div
      className="absolute"
      style={{
        ...
        borderRadius: "9px",
        zIndex: 4,
        background: "linear-gradient(90deg, #8e5109 0%, #cf8520 46%, #7f4707 100%)",
        border: "1px solid #764205",
        boxShadow:
          "inset -2px 0 0 rgba(255,255,255,0.22), inset 2px 0 0 rgba(0,0,0,0.18), 0 2px 2px rgba(90,48,4,0.18)",
      }}
    >
      <div
        className="absolute"
...
function IntroCorral({ x, y }: { x: number; y: number }) {
  return (
    ...
    >
      ...
      >
        <IntroFenceRun width={116} left={18} top={18} showStartPost={false} showEndPost={false} />
        <IntroFenceRun width={116} left={206} top={18} showStartPost={false} showEndPost={false} />
        <IntroFenceRun width={172} left={-52} top={102} rotate={90} showStartPost={false} showEndPost={false} />
        <IntroFenceRun width={172} left={220} top={102} rotate={90} showStartPost={false} showEndPost={false} />
        <IntroFenceRun width={304} left={18} top={184} showStartPost={false} showEndPost={false} />
        <IntroCornerPost left={16} top={17} />
        <IntroCornerPost left={300} top={17} />
        <IntroCornerPost left={16} top={182} />
        <IntroCornerPost left={300} top={182} />
      </div>
    </div>
  );
}
...
export default function App() {
  const [gameSize, setGameSize] = useState(() =>
    typeof window === "undefined" ? { width: GAMEPLAY_WIDTH, height: 700 } : getGameplaySize(getViewportSize())
  );
  const [viewportSize, setViewportSize] = useState(() =>
    typeof window === "undefined" ? { width: GAMEPLAY_WIDTH, height: 700 } : getViewportSize()
  );

  const [renderState, setRenderState] = useState({
    pigX: gameSize.width / 2 - PIG_SIZE / 2,
    pigDirection: 1,
    score: 0,
    lastScore: 0,
    topScore: 0,
  ...
  const continueAdPendingRef = useRef(false);
  ...
  const gameScale = getGameplayScale(viewportSize.width);
  ...
  const pressClass = (id: string) => (pressedButtonId === id ? "is-pressed" : "");

  const markButtonPressed = useCallback((id: string) => {
    setPressedButtonId(id);
    window.setTimeout(() => {
      setPressedButtonId((current) => (current === id ? null : current));
  ...
  useEffect(() => {
    const onResize = () => {
      ...
      const nextGameSize = getGameplaySize(nextViewport);
      setViewportSize(nextViewport);
      setGameSize(nextGameSize);
      setIsLandscape(nextViewport.width > nextViewport.height);

      const state = gameStateRef.current;
      state.pigX = Math.min(nextGameSize.width - PIG_SIZE, Math.max(0, state.pigX));
    };
  ...
  const resetRun = useCallback(() => {
    ...
    state.pigX = gameSize.width / 2 - PIG_SIZE / 2;
  ...
  const handleRewardedContinue = useCallback(async () => {
    if (continueAdPendingRef.current) return;

    continueAdPendingRef.current = true;
    try {
      const rewarded = await showRewardedContinueAd();
      if (rewarded) {
        startCountdown("continue");
      }
    } finally {
      continueAdPendingRef.current = false;
    }
  }, [startCountdown]);

  useEffect(() => {
    if (renderState.canContinueFromGameOver) {
      prepareRewardedContinueAd();
    }
  }, [renderState.canContinueFromGameOver]);
  ...
  useEffect(() => {
    ...
    const gameLoop = (timestamp: number) => {
      ...
      if (hasStarted && !runIntro.active && !state.isGameOver && !isPaused && !showExitConfirm && countdown === null && !isLandscape) {
      ...
      const canContinueFromGameOver = canRestartFromGameOver && !state.hasUsedContinue;
    ...
    animationFrameId = requestAnimationFrame(gameLoop);
    return () => cancelAnimationFrame(animationFrameId);
  }, [playTone, playPigHitSound, triggerVibration, isPaused, showExitConfirm, countdown, hasStarted, runIntro.active, isLandscape, gameSize.width, gameSize.height, PIG_Y]);
  ...
  return (
    <div className="relative w-screen h-screen bg-sky-300 overflow-hidden">
      ...
      >
      <div
        className="relative border-4 border-amber-900 overflow-hidden w-full h-full"
      ...
      >
        ...
        {!hasStarted && (
          ...
          >
            <img src={startScreenImage} alt="Fugitive Pig start screen" className="absolute inset-0 w-full h-full object-cover" />
            <div className="absolute inset-0 bg-black/25" />

            <div className="absolute inset-0 flex items-end justify-center pb-20">
              <button
                type="button"
                className={`pressable-button ${pressClass("start")} rounded-2xl bg-transparent p-0 border-0 shadow-none`}
                onPointerDown={() => markButtonPressed("start")}
                onClick={(e) => {
                  e.stopPropagation();
                  beginRunIntro();
                }}
        ...
        {hasStarted && (runIntro.active || isPaused || countdown !== null || renderState.isGameOver) && (
        ...
        {!renderState.isGameOver && countdown === null && !runIntro.active && !isPaused && !showExitConfirm && hasStarted && !isLandscape && (
          <button
            type="button"
            className={`pressable-button ${pressClass("pause")} absolute top-2 right-2 z-30 w-14 h-14 appearance-none bg-transparent border-0 p-0 m-0 shadow-none outline-none`}
            style={{ WebkitTapHighlightColor: "transparent" }}
            onPointerDown={(e) => {
              e.preventDefault();
              e.stopPropagation();
              markButtonPressed("pause");
            }}
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
        ...
        <GameUI
          ...
          onContinue={handleRewardedContinue}
        />
          ...
          >
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  className={`pressable-button ${pressClass("exit-no")} h-[62px] rounded-2xl bg-transparent p-0 border-0 shadow-none`}
                  onPointerDown={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    markButtonPressed("exit-no");
                  }}
                ...
                <button
                  type="button"
                  className={`pressable-button ${pressClass("exit-yes")} h-[62px] rounded-2xl bg-transparent p-0 border-0 shadow-none`}
                  onPointerDown={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    markButtonPressed("exit-yes");
                  }}
                  onClick={() => {
