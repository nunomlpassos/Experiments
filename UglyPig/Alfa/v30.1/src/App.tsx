...
function FenceHitboxDebug({ fence, gameWidth }: { fence: FenceData; gameWidth: number }) {
  ...
  const hitRects: RectHitbox[] = [
    { x: 0, y: fence.y, w: Math.max(0, fence.gapStart), h: FENCE_HEIGHT },
    { x: gapRight, y: fence.y, w: Math.max(0, gameWidth - gapRight), h: FENCE_HEIGHT },
  ];
    ...
    <>
      {hitRects.map((rect, index) =>
        rect.w <= 0 ? null : (
          <div
            key={`${fence.id}-${index}`}
            className="absolute pointer-events-none z-20 bg-red-500/25 border border-red-600"
            style={{
              left: `${rect.x}px`,
              top: `${rect.y}px`,
              width: `${rect.w}px`,
    ...
    const gameLoop = (timestamp: number) => {
      ...
      if (hasStarted && !state.isGameOver && !isPaused && countdown === null && !isLandscape) {
        ...
        state.fences.forEach((fence) => {
          ...
          const hitLeft = leftFenceRect.w > 0 && pigMaskIntersectsRect(state.pigX, PIG_Y, PIG_SIZE, state.pigDirection, leftFenceRect);
          const hitRight = rightFenceRect.w > 0 && pigMaskIntersectsRect(state.pigX, PIG_Y, PIG_SIZE, state.pigDirection, rightFenceRect);
