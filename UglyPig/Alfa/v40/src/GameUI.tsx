...
const BEST_CONFETTI = Array.from({ length: 24 }, (_, i) => i);
...
function getCelebrationDelays(newRecords: PersonalRecordFlags, isNewBest: boolean) {
  ...
  if (isNewBest) delays.best = nextDelay + 0.08;
  return delays;
}
...
function CelebrationStyles() {
  return (
    <style>{`
      ...
      .pr-daily-card {
        animation: prDailyCard .82s cubic-bezier(.2,.85,.3,1.25) var(--pr-delay) both;
      }
      .pr-weekly-card {
        animation: prWeeklyCard 1.05s cubic-bezier(.2,.85,.3,1.25) var(--pr-delay) both;
      }
      .pr-monthly-card {
        animation: prMonthlyCard 1.25s cubic-bezier(.18,.88,.25,1.3) var(--pr-delay) both;
      }
      .pr-best-card {
        animation: prBestCard 1.5s cubic-bezier(.16,.9,.22,1.28) var(--pr-delay) both;
      }
      .pr-daily .pr-halo {
        animation: prHalo .75s ease-out var(--pr-delay) both;
      }
      .pr-weekly .pr-halo {
        animation: prHalo 1s ease-out var(--pr-delay) both;
      }
      .pr-weekly .pr-ring {
        animation: prRing 1.05s ease-out var(--pr-delay) both;
      }
      .pr-monthly .pr-halo {
        animation: prHalo 1.2s ease-out var(--pr-delay) both;
      }
      .pr-monthly .pr-ring {
        animation: prRing 1.2s ease-out var(--pr-delay) both;
      }
      .pr-monthly .pr-burst {
        animation: prBurst 1.25s ease-out var(--pr-delay) both;
      }
      .pr-best .pr-halo {
        inset: -12px;
        animation: prBestHalo 1.5s ease-out var(--pr-delay) both;
      }
      .pr-best .pr-ring {
        inset: -4px;
        border-width: 4px;
        animation: prBestRing 1.5s ease-out var(--pr-delay) both;
      }
      .pr-best .pr-burst {
        inset: -38px;
        animation: prBestBurst 1.6s ease-out var(--pr-delay) both;
      }
      .pr-daily .pr-spark,
      .pr-weekly .pr-spark,
      .pr-monthly .pr-spark,
      .pr-best .pr-spark {
        animation: prSpark .78s ease-out calc(var(--pr-delay) + var(--spark-delay)) both;
      }
      .pr-monthly-flash {
        position: absolute;
        inset: 0;
        ...
        opacity: 0;
        animation: prScreenFlash .85s ease-out var(--pr-delay) both;
      }
      .pr-best-confetti {
        position: absolute;
        left: 50%;
        ...
        animation: prBestConfetti 1.7s cubic-bezier(.16,.75,.26,1) calc(var(--pr-delay) + var(--confetti-delay)) both;
      }
      .pr-new-best-ribbon {
        animation: prRibbon .72s cubic-bezier(.2,.9,.25,1.35) calc(var(--pr-delay) + .38s) both;
      }
      ...
      @keyframes prRibbon {
        0% { transform: translate(-50%, 7px) scale(.55); opacity: 0; }
            ...
            style={
              {
                ...
                "--confetti-y": `${Math.sin(angle) * radius + 72}px`,
  ...
  return (
    <div className={compact ? "mb-3" : "mb-4"}>
      ...
      <div className="grid grid-cols-3 gap-2">
        {items.map((item) => (
          ...
          >
            ...
            {item.isNew && (
              <div className="pr-card-content absolute -top-2 left-1/2 -translate-x-1/2 rounded-full bg-[#e3a008] px-2 py-[1px] text-[8px] font-black text-white leading-none">
                NEW
              </div>
            )}
      ...
      >
        ...
        {isNewBest && (
          <div
            className="pr-new-best-ribbon absolute -bottom-3 left-1/2 z-20 -translate-x-1/2 whitespace-nowrap rounded-md border-2 border-[#8b1c0e] bg-[linear-gradient(#f15837,#c51f12)] px-3 py-[2px] text-[9px] font-black tracking-wide text-white shadow-lg"
            style={{ "--pr-delay": `${celebrationDelays?.best ?? 0}s` } as CSSProperties}
          >
            NEW BEST
          </div>
        )}
          ...
          {canRestartFromGameOver && (
            canContinueFromGameOver ? (
              <div className="grid grid-cols-2 gap-3 items-center">
                <button type="button" onClick={onContinue} className="bg-transparent border-0 p-0 block">
                  <img src={continueButton} alt="Continue" className="w-full h-auto" draggable={false} />
                </button>
                <button type="button" onClick={onRestart} className="bg-transparent border-0 p-0 block">
