...
if (
  !levelIntro ||
  levelIntro.previewDuration !== levelPreviewDuration("full") ||
  levelIntro.actionDuration !== LEVEL_INTRO_DURATION_MS ||
  levelIntro.duration !== levelIntro.previewDuration + levelIntro.actionDuration ||
...
animateLevelIntro(levelIntro.previewDuration * 0.5);
const previewGeometry = boardGeometry(500, 400);
const previewCamera = mazeCameraState(previewGeometry, 500, 400);
if (
  levelIntroActionProgress() !== 0 ||
  Math.abs(previewCamera.zoom - 1) > 0.001 ||
  Math.abs(previewCamera.offset.x) > 0.001 ||
  Math.abs(previewCamera.offset.y) > 0.001
) {
  throw new Error("Level preview did not keep the complete maze visible");
}

animateLevelIntro(levelIntro.previewDuration + levelIntro.actionDuration * 0.4);
...
levelIntro.progress =
  (levelIntro.previewDuration + levelIntro.actionDuration * 0.4) / levelIntro.duration;
...
animateLevelIntro(levelIntro.duration);
if (
  levelIntro ||
  moveUpButton.disabled ||
  !powerControlsEnabled ||
...
if (
  !levelIntro ||
  levelIntro.previewDuration !== LEVEL_PREVIEW_RETRY_MS ||
  levelIntro.actionDuration !== LEVEL_INTRO_RETRY_DURATION_MS ||
  levelIntro.duration !== LEVEL_PREVIEW_RETRY_MS + LEVEL_INTRO_RETRY_DURATION_MS
) {
...
if (
  ...
  !ATTEMPT_STORAGE_KEY.includes("V31") ||
  !CAMPAIGN_STORAGE_KEY.includes("V31") ||
  !ECONOMY_STORAGE_KEY.includes("V31")
) {
  throw new Error("V3.1 camera did not preserve the strict centered three-cell view");
}
...
console.log("Validated timed full-board preview and continuous transition into the close camera.");
