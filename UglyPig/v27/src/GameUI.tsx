...
function GameUIComponent({
  ...
  return (
    <div className="absolute inset-0 bg-black/55 flex flex-col items-center justify-center z-40 px-4">
      <div className="relative w-[92%] max-w-[360px] rounded-[28px] border-[6px] border-lime-500 bg-[#f7f0df] p-5 shadow-2xl">
        <div className="absolute -top-7 left-1/2 -translate-x-1/2 rounded-2xl bg-[#b96a2b] px-8 py-2 text-white text-4xl font-black tracking-wide border-4 border-[#8e4c18]">PAUSED</div>
        <div className="pt-10 flex flex-col gap-3 mb-4">
          <div className="flex justify-between items-center border-b border-[#d8c9a8] pb-2">
            ...
            <span className="text-[#5a3216] text-3xl font-black">{score}</span>
          </div>
          <div className="flex justify-between items-center border-b border-[#d8c9a8] pb-2">
            <span className="text-slate-600 text-2xl font-black">Last Run:</span>
            <span className="text-slate-700 text-3xl font-black">{lastScore}</span>
          </div>
          <div className="flex justify-between items-center border-b border-[#d8c9a8] pb-2">
            <span className="text-green-700 text-2xl font-black">Best:</span>
            <span className="text-green-700 text-3xl font-black">{topScore}</span>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3 mb-3">
          <button type="button" onClick={onToggleSound} className="flex items-center justify-center gap-2 px-3 py-3 rounded-2xl bg-[#10295c] text-white font-black text-xl border-2 border-[#263f77]" title={soundEnabled ? "Sound ON" : "Sound OFF"}>
            <span>{soundEnabled ? "🔊" : "🔇"}</span><span>{soundEnabled ? "ON" : "OFF"}</span>
          </button>
          <button type="button" onClick={onToggleVibration} className="flex items-center justify-center gap-2 px-3 py-3 rounded-2xl bg-[#10295c] text-white font-black text-xl border-2 border-[#263f77]" title={vibrationEnabled ? "Vibration ON" : "Vibration OFF"}>
            <span>{vibrationEnabled ? "📳" : "📴"}</span><span>{vibrationEnabled ? "ON" : "OFF"}</span>
          </button>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <button type="button" onClick={onResume} className="px-3 py-3 rounded-2xl bg-lime-600 text-white font-black text-2xl border-2 border-lime-700">Resume</button>
          <button type="button" onClick={onRestart} className="px-3 py-3 rounded-2xl bg-red-600 text-white font-black text-2xl border-2 border-red-700">Restart</button>
        </div>
      </div>
    </div>
  );
