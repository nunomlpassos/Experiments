...
const ATTEMPT_STORAGE_KEY = "mouseMazeAttemptStateV31Preview1";
const CAMPAIGN_STORAGE_KEY = "mouseMazeCampaignStateV31Preview1";
...
const LEVEL_PREVIEW_MAX_MS = 2000;
const LEVEL_PREVIEW_MIN_MS = 1200;
const LEVEL_PREVIEW_LEVEL_STEP_MS = 90;
const LEVEL_PREVIEW_RETRY_MS = 700;
const LEVEL_CAMERA_ZOOM_RATIO = 0.32;
...
const ECONOMY_STORAGE_KEY = "mouseMazeEconomyStateV31Preview1";
const POWER_CONFIGS = {
  crystal: {
    name: "Crystal Vision",
    price: 10,
...
function drawMaze() {
  ...
  mazeContext.save();
  mazeContext.translate(camera.offset.x, camera.offset.y);
...
function mazeCameraState(geometry, width, height) {
  ...
  const center = { x: width * 0.5, y: height * CAMERA_VERTICAL_CENTER };
  const amount = levelIntroCameraAmount();
  const zoom = 1 + (CAMERA_ZOOM - 1) * amount;
  const finalOffset = {
    x: center.x - focus.x * CAMERA_ZOOM,
    y: center.y - focus.y * CAMERA_ZOOM,
  };
  return {
    center,
    focus,
    ...
    amount,
    offset: {
      x: finalOffset.x * amount,
      y: finalOffset.y * amount,
    },
  };
}
...
function introDoorGeometry(outer, lineWidth, depth) {
  ...
  const openAmount = levelIntroStateAt(levelIntroActionProgress()).doorOpen;
...
function currentMouseVisualState() {
  if (levelIntro) {
    const state = levelIntroStateAt(levelIntroActionProgress());
...
function drawCharacters(ctx, corners) {
  ...
  const activeSniffProgress = levelIntro
    ? levelIntroStateAt(levelIntroActionProgress()).sniffProgress
  ...
  if (levelIntro) {
    const introState = levelIntroStateAt(levelIntroActionProgress());
    if (
...
function levelPreviewDuration(mode = "full") {
  if (mode === "retry") return LEVEL_PREVIEW_RETRY_MS;
  return clamp(
    LEVEL_PREVIEW_MAX_MS - (level - 1) * LEVEL_PREVIEW_LEVEL_STEP_MS,
    LEVEL_PREVIEW_MIN_MS,
    LEVEL_PREVIEW_MAX_MS,
  );
}

function levelIntroActionProgress(intro = levelIntro) {
  if (!intro) return 1;
  const elapsed = intro.progress * intro.duration;
  return clamp((elapsed - intro.previewDuration) / intro.actionDuration, 0, 1);
}

function levelIntroCameraAmount() {
  if (!levelIntro) return 1;
  const actionProgress = levelIntroActionProgress();
  return smoothStep(clamp(actionProgress / LEVEL_CAMERA_ZOOM_RATIO, 0, 1));
}
...
function animateLevelIntro(timestamp) {
  ...
  if (!levelIntro.actionStarted && levelIntroActionProgress() > 0) {
    levelIntro.actionStarted = true;
    setMessage("The maze door is opening...");
    mazeEl.setAttribute("aria-label", "Mouse entering the maze");
  }
  drawMaze();
...
function startLevelIntroAnimation(mode = "full") {
  clearLevelIntroAnimation();
  const previewDuration = levelPreviewDuration(mode);
  ...
  levelIntro = {
    ...
    previewDuration,
    actionDuration,
    duration: previewDuration + actionDuration,
    actionStarted: false,
    start: { ...levelStart },
  };
  ...
  setMessage(
    mode === "retry"
      ? "One more look before the mouse heads back in..."
      : "Study the full maze before the close view begins...",
  );
  mazeEl.setAttribute("aria-busy", "true");
  updatePowerUI();
  requestMazeDraw();
