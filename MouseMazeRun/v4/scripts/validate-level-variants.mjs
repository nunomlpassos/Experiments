...
globalThis.__variantValidation = LEVEL_CONFIGS.map((config, levelIndex) => {
  ...
  return { level: levelIndex + 1, variants };
});
...
if (
  ...
  movesLeft !== retryExpected.moveLimit ||
  !hammerAvailable ||
  powerInventory.hammer !== 2
) {
...
moveLimit = persistedVariant.moveLimit;
movesLeft = 12;
...
for (let row = 0; row < ROWS && !openedPersistedWall; row += 1) {
...
if (
  ...
  movesLeft !== persistedRestartVariant.moveLimit ||
  maze
    .flat()
    .map((cell) =>
      [cell.walls.top, cell.walls.right, cell.walls.bottom, cell.walls.left]
...
levelIntro.start = { row: ROWS - 1, col: COLS - 1 };
...
mouse = { ...START };
movesLeft = crystalVariant.moveLimit;
gameOver = false;
campaignComplete = false;
powerInventory = createEmptyPowerInventory();
powerInventory.crystal = 1;
