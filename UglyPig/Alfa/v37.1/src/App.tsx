...
const FENCES_PER_DIFFICULTY_LEVEL = 5;
const GAME_VERSION = "v37.1";
...
const getDifficultyLevelForScore = (score: number) =>
  Math.min(DIFFICULTY_LEVELS.length, Math.floor(score / FENCES_PER_DIFFICULTY_LEVEL) + 1);
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      gameStateRef.current.isFirstInstallRun = localStorage.getItem(FIRST_INSTALL_RUN_COMPLETE_KEY) !== "true";
        ...
        state.fences.forEach((fence) => {
          ...
          if (!fence.passed && fence.y > PIG_Y + PIG_SIZE) {
            ...
            if (state.isFirstInstallRun) {
              const nextDifficultyLevel = getDifficultyLevelForScore(state.score);
  ...
  return (
    <div className="w-screen h-screen bg-sky-300 overflow-hidden">
      ...
      >
        ...
        {hasStarted && (
          <div className="absolute top-2 left-2 z-30 rounded-lg bg-black/55 px-3 py-2 text-xs font-black text-white shadow-lg pointer-events-none">
            NIVEL {renderState.difficultyLevel}
          </div>
        )}
