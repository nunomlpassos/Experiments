...
const CRYSTAL_TUTORIAL_STORAGE_KEY = "mouseMazeCrystalTutorialV45";
const TORNADO_TUTORIAL_STORAGE_KEY = "mouseMazeTornadoTutorialV45";
const POWER_INTRO_TUTORIAL_STORAGE_KEY = "mouseMazePowerIntroTutorialV45Flow2";
const TUNNEL_TUTORIAL_STORAGE_KEY = "mouseMazeTunnelTutorialV45";
const ROTATING_TILES_TUTORIAL_STORAGE_KEY = "mouseMazeRotatingTilesTutorialV45";
const ATTEMPT_STORAGE_KEY = "mouseMazeAttemptStateV45World1";
const CAMPAIGN_STORAGE_KEY = "mouseMazeCampaignStateV45Dynamic1";
const WORLD_PROGRESS_STORAGE_KEY = "mouseMazeWorldProgressV45Dynamic1";
...
const ECONOMY_STORAGE_KEY = "mouseMazeEconomyStateV45World1";
const POWER_GRANT_STORAGE_KEY = "mouseMazePowerGrantsV45Dynamic1";
const POWER_CONFIGS = {
  crystal: {
    name: "Crystal Vision",
    price: 10,
...
const LEVEL_CONFIGS = [
  ...
  chapterLevel(35533, "Md", {
    ...
    rockMode: "blocker",
    rockCount: 2,
    adversityVariants: repeatedAdversity("cockroach"),
    minPath: 30,
    maxPath: 48,
    minTurns: 11,
    moveMargin: 7,
...
const INTRO_LEVELS_WITHOUT_ADVERSITY = new Set([13, 14, 19, 30, 33, 37, 40]);
...
const WORLD_LEVELS = [
  { x: 20, y: 95, objective: "Learn how the mouse moves", difficulty: "Tutorial - 5 x 5" },
  { x: 49, y: 88, objective: "Find the cheese", difficulty: "Very Easy" },
  { x: 78, y: 81, objective: "Tip the bottle and drink the milk", difficulty: "Easy" },
  { x: 62, y: 74, objective: "Collect cheese and spilled milk", difficulty: "Normal" },
  { x: 28, y: 67, objective: "Collect three pantry treats", difficulty: "Easy" },
  { x: 18, y: 60, objective: "Plan the order of the milk objectives", difficulty: "Normal" },
  { x: 50, y: 53, objective: "Discover Crystal Vision", difficulty: "Difficult" },
  { x: 81, y: 46, objective: "Remember the revealed route", difficulty: "Difficult" },
  { x: 67, y: 39, objective: "Escape the remote-control catcher", difficulty: "Very Difficult" },
  { x: 34, y: 32, objective: "Outrun the catcher and collect every cheese", difficulty: "Very Difficult" },
  { x: 18, y: 25, objective: "Push the rock to reveal the objective", difficulty: "Easy" },
  { x: 46, y: 18, objective: "Search the rocks while escaping the catcher", difficulty: "Extreme" },
  { x: 77, y: 11, objective: "Discover the Maze Tornado", difficulty: "Very Difficult" },
  { x: 57, y: 4.5, objective: "Master the first hinged wall", difficulty: "Very Difficult" },
  { x: 20, y: 95, objective: "Discover the cheese-carrying cockroach", difficulty: "Difficult" },
  { x: 48, y: 89, objective: "Protect the milk from the cockroach", difficulty: "Normal" },
  { x: 78, y: 83, objective: "Discover the wall-breaking hammer", difficulty: "Very Difficult" },
  { x: 66, y: 76, objective: "Adapt to hinged walls under pressure", difficulty: "Very Difficult" },
  { x: 32, y: 69, objective: "Eat the pie from all four sides", difficulty: "Difficult" },
  { x: 18, y: 62, objective: "Collect the milk and cheese", difficulty: "Easy" },
  { x: 46, y: 55, objective: "Discover the first shaped maze", difficulty: "Normal" },
  { x: 80, y: 48, objective: "Search the pantry rocks under pressure", difficulty: "Very Difficult" },
  { x: 68, y: 41, objective: "Discover the fishing rod", difficulty: "Very Difficult" },
  { x: 36, y: 34, objective: "Eat the pie while evading the catcher", difficulty: "Extreme" },
  { x: 18, y: 27, objective: "Discover the cat paw striking from the frame", difficulty: "Very Difficult" },
  { x: 49, y: 20, objective: "Adapt to hinged walls and a moving objective", difficulty: "Extreme" },
  { x: 79, y: 13, objective: "Discover the rocket and protect every objective", difficulty: "Extreme" },
  { x: 57, y: 4.5, objective: "Outrun the catcher while evading the cat paw", difficulty: "Insane" },
  { x: 23, y: 94, objective: "Recall the pantry mechanics", difficulty: "Normal" },
  { x: 55, y: 86, objective: "Discover the transport tunnels", difficulty: "Difficult" },
  { x: 81, y: 78, objective: "Use the tunnels while under pressure", difficulty: "Difficult" },
  { x: 62, y: 70, objective: "Adapt to moving objectives and pantry rocks", difficulty: "Very Difficult" },
  { x: 29, y: 62, objective: "Discover the rotating floor circuits", difficulty: "Difficult" },
  { x: 18, y: 54, objective: "Cross the rotating floor under pressure", difficulty: "Extreme" },
  { x: 48, y: 46, objective: "Take cover from the diving crow", difficulty: "Extreme" },
  { x: 79, y: 38, objective: "Master tunnels, rotating floors, and two threats", difficulty: "Insane" },
  { x: 63, y: 30, objective: "Follow the route through moving clouds", difficulty: "Difficult" },
  { x: 31, y: 23, objective: "Navigate while the clouds cross the pantry", difficulty: "Very Difficult" },
  { x: 18, y: 16, objective: "Move the rocks under pressure", difficulty: "Extreme" },
  { x: 43, y: 10, objective: "Discover the rising flood", difficulty: "Extreme" },
  { x: 72, y: 6, objective: "Outrun the flood through the dead ends", difficulty: "Insane" },
  { x: 57, y: 3, objective: "Master the pantry under shifting cover", difficulty: "Very Difficult" },
];
...
function renderWorldMap() {
  const chapterTrackHeight = 920;
  const chapterOneLevelCount = 14;
  const chapterTwoLevelCount = 28;
  const currentLevel = clamp(worldProgress.unlockedLevel, 1, WORLD_LEVELS.length);
  const currentMeta = WORLD_LEVELS[currentLevel - 1];
  const secondSectionUnlocked = worldProgress.unlockedLevel > chapterOneLevelCount;
  const thirdSectionUnlocked = worldProgress.unlockedLevel > chapterTwoLevelCount;
  const visibleStart = 1;
  const visibleEnd = thirdSectionUnlocked
    ? WORLD_LEVELS.length
    : secondSectionUnlocked
      ? chapterTwoLevelCount
      : chapterOneLevelCount;
  const visibleLevels = WORLD_LEVELS.slice(visibleStart - 1, visibleEnd);
  const completedCount = worldProgress.completedLevels
    .slice(0, WORLD_LEVELS.length)
    .filter(Boolean).length;

  const levelMapY = (levelNumber, meta) => {
    const localY = (meta.y / 100) * chapterTrackHeight;
    if (thirdSectionUnlocked) {
      if (levelNumber <= chapterOneLevelCount) return chapterTrackHeight * 2 + localY;
      if (levelNumber <= chapterTwoLevelCount) return chapterTrackHeight + localY;
      return localY;
    }
    if (secondSectionUnlocked && levelNumber <= chapterOneLevelCount) {
      return chapterTrackHeight + localY;
    }
    return localY;
  };

  const currentChapter = currentLevel > chapterTwoLevelCount
    ? 3
    : currentLevel > chapterOneLevelCount
      ? 2
      : 1;
  worldChapterLabelEl.textContent = `Chapter ${currentChapter}`;
  worldChapterTitleEl.textContent = currentChapter === 3
    ? "Behind the skirting boards"
    : currentChapter === 2
      ? "Beyond the pantry"
      : "Through the cupboards";
  worldMapTrackEl.dataset.expanded = String(secondSectionUnlocked);
  worldMapTrackEl.dataset.third = String(thirdSectionUnlocked);
  ...
  worldProgressTextEl.textContent = `${completedCount} / ${WORLD_LEVELS.length}`;
  worldProgressBarEl.style.transform = `scaleX(${completedCount / WORLD_LEVELS.length})`;
...
function currentCrystalPath() {
  if (waterIsActive()) {
    ...
    const safeRoutes = targets
      .map((target) => findWaterSafePath(mouse, target))
...
function chooseSafeTornadoCandidate() {
  const config = LEVEL_CONFIGS[level - 1];
  const remaining = remainingCheeseTargets();
  if (!config || (!remaining.length && !remainingMilkBottles().length)) return null;
  ...
  for (const variantIndex of variantIndices) {
    ...
    const route = milkBottles.length
      ? (() => {
    ...
    if (route?.path.length) {
      const candidate = {
        ...
        routeDistance: route.path.length - 1,
        variantIndex,
        hingedWalls: variant.hingedWalls ?? [],
      };
      if (!bestCandidate || candidate.routeDistance < bestCandidate.routeDistance) {
...
function finishCatPawAnimation() {
  ...
  setMovementControlsEnabled(true);
  setPowerControlsEnabled(true);
  setMessage("The paw missed. Keep moving before the cat tries again.");
  saveCampaignState();
...
function advanceCrowClockAfterMouseMove() {
  ...
  if (remaining <= 2 && !crowThreat) crowThreat = chooseCrowThreat();
  if (remaining === 0) crowStrikePending = true;
  requestMazeDraw();
}
...
function startCrowAnimation(queuedDirection = null) {
  ...
  const caught = Boolean(targetedCell && !blockedByWall);
  crowAnimation = {
    threat,
    caught,
...
function waterExpansionKeys(floodedKeys = floodedWaterKeys, rocks = rockPositions) {
  ...
  for (const floodedKey of floodedKeys) {
    const floodedCell = cellFromKey(floodedKey);
    const mazeCell = maze[floodedCell.row]?.[floodedCell.col];
    if (!mazeCell) continue;
    for (const next of neighbors(maze, mazeCell)) {
      const nextKey = keyOf(next);
      if (floodedKeys.has(nextKey) || rockAt(next, rocks)) continue;
...
function waterArrivalMoves(rocks = rockPositions) {
  const arrivalMoves = new Map([[keyOf(levelStart), WATER_PREPARATION_MOVES]]);
...
function waterAdvanceForNextMove(rocks = rockPositions) {
  ...
  if (moveCount < WATER_PREPARATION_MOVES) return { moveCount, newKeys: [] };
  if (moveCount === WATER_PREPARATION_MOVES) {
    return { moveCount, newKeys: [keyOf(levelStart)] };
  }
  return { moveCount, newKeys: waterExpansionKeys(floodedWaterKeys, rocks) };
}
...
function commitWaterAdvance(advance) {
...
function waterWarningKeys() {
  ...
  if (!floodedWaterKeys.size) return [keyOf(levelStart)];
...
function milkBottleAt(cell, bottles = milkBottles) {
  if (!cell) return null;
  return bottles.find((bottle) => bottle.row === cell.row && bottle.col === cell.col) ?? null;
}
...
function nextCarStepForTurn(grid, carPosition, mousePosition) {
  if (hasOpenPassageBetween(grid, carPosition, mousePosition)) {
    return { ...carPosition };
  }
...
function addCarAdversityToPuzzle(result, grid, start, rng) {
  ...
  const carStart = candidates[0];
  return carStart
    ? { ...result, car: { id: "catcher", row: carStart.row, col: carStart.col } }
    : null;
}
...
function addRotatingCircuitsToPuzzle(result, grid, start, config, rng) {
  ...
  return {
    ...
    path: route.path,
    moveLimit: Math.max(result.moveLimit ?? 0, route.path.length - 1 + 12),
    rotatingCircuits: [
      {
...
function buildLevelVariant(config, variantIndex) {
  ...
  if (pregenerated) {
    ...
    return { ...pregenerated, activeAdversities: variantConfig.activeAdversities };
  }
  setGridDimensions(variantConfig);
  ...
  for (const attempt of attempts) {
    ...
    if (
      ...
      result = addCarAdversityToPuzzle(result, grid, start, rng);
    }
...
function waterSourceForCell(cell, wetKeys = floodedWaterKeys) {
  ...
  return keyOf(cell) === keyOf(levelStart) ? waterOutsideSource(cell) : null;
}
...
function drawMilkBottle(ctx, bottle, corners, cellWidth) {
  ...
  const visualBottle = rotatingVisualCell("milk", bottle.id, bottle);
...
function drawCrowStrike(ctx, corners, width) {
  ...
  const { threat, progress, targetedCell, blockedByWall } = crowAnimation;
  ...
  const center = {
    x:
      ...
      (beforeImpact
        ? -(1 - approach) * flightHorizontal
        : departure * flightHorizontal * 1.18) +
      Math.sin(progress * Math.PI * 6) * drawWidth * 0.018,
    y:
      targetGeometry.center.y +
      (beforeImpact
        ? (1 - approach) * flightVertical
        : -departure * flightVertical * 1.18),
  };
  ...
  ctx.save();
  ctx.translate(center.x, center.y);
  ctx.rotate(-0.08 + Math.sin(progress * Math.PI * 4) * 0.025);
...
function drawCharacters(ctx, corners) {
  ...
  for (const target of remainingCheeses) {
    ...
    const visualTarget = rotatingVisualCell("cheese", cheeseIdentity(target), target);
...
function createRotatingTilesTurnState(tutorial = false, nextTurnState = {}) {
  ...
  for (const target of cheeseTargets) {
    if (!cheeseWasCollected(target)) {
      addRotatingEntityMotion(entityMotions, destinations, "cheese", cheeseIdentity(target), target);
    }
  }
  for (const bottle of milkBottles) {
    addRotatingEntityMotion(entityMotions, destinations, "milk", bottle.id, bottle);
    if (bottle.spill) {
...
function startSynchronizedRotatingTilesTurn(target, rockPush, nextCar) {
...
function findTornadoTutorialOpportunity() {
  ...
  const candidate = chooseSafeTornadoCandidate();
...
function finishLevelReadyState() {
  ...
  else if (config.catPawMode && level === 25) {
  ...
  else if (config.crowMode && level === 35) {
    setMessage("The crow hunts open ground. A square touching any wall is safe from its claws.");
  }
...
function finishMouseMotion() {
  ...
  if (currentLevelConfig().cockroachMode) cockroachMovesSinceAction += 1;
...
function finishMilkKnockAnimation() {
  ...
  noteCloudPlayerAction();
  mazeEl.setAttribute("aria-busy", "false");
  const waterLoss = commitWaterAdvance(animation.waterAdvance);
  render();
  if (waterLoss) {
    loseLevel(waterLoss);
    return;
...
function startMouseMotion(target, rockPush = null, hingedWallTrigger = null) {
  ...
  if (car) {
    carMotion = {
      from: { ...car },
      to: nextCarStepForTurn(maze, car, mouse),
      progress: 0,
      startedAt: null,
    };
    carFacingDirection = carDirectionBetween(
...
function move(directionKey) {
  ...
  if (
    ...
    activeTunnelAt(rockDestination) ||
