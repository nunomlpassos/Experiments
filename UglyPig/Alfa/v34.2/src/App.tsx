...
const PIG_SPEED_X = 3.564; // 10% slower than v31
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const gameLoop = (timestamp: number) => {
      ...
      const frameScale = Math.min(2, Math.max(0, (timestamp - previousFrameTime) / TARGET_FRAME_MS));
      ...
      if (hasStarted && !state.isGameOver && !isPaused && countdown === null && !isLandscape) {
        state.pigX += state.pigDirection * PIG_SPEED_X * frameScale;
        ...
        state.fences = state.fences.map((f) => ({ ...f, y: f.y + FENCE_SPEED * frameScale })).filter((f) => f.y < gameSize.height);
