          ...
          {canRestartFromGameOver && (
            canContinueFromGameOver ? (
              <div className="grid grid-cols-2 gap-3 items-center">
                <button type="button" onPointerDown={() => markButtonPressed("continue")} onClick={onContinue} className={`pressable-button ${pressClass("continue")} bg-transparent border-0 p-0 block`}>
                  <img src={continueButton} alt="Continue" className="w-full h-auto" draggable={false} />
                </button>
                <button type="button" onPointerDown={() => markButtonPressed("play-again")} onClick={onRestart} className={`pressable-button ${pressClass("play-again")} bg-transparent border-0 p-0 block`}>
                  <img src={playAgainButton} alt="Play again" className="w-full h-auto" draggable={false} />
                </button>
              </div>
            ) : (
              <button type="button" onPointerDown={() => markButtonPressed("play-again")} onClick={onRestart} className={`pressable-button ${pressClass("play-again")} w-[48%] mx-auto bg-transparent border-0 p-0 block`}>
                <img src={playAgainButton} alt="Play again" className="w-full h-auto" draggable={false} />
              </button>
            )
          )}
    ...
    <div className="absolute inset-0 bg-black/55 flex flex-col items-center justify-center z-40 px-4">
      <div className="relative w-[90%] max-w-[352px] rounded-[28px] p-[6px] shadow-2xl bg-[linear-gradient(to_bottom,#A6CE12_0%,#88A808_45%,#789808_70%,#588008_100%)]">
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <button type="button" onPointerDown={() => markButtonPressed("sound")} onClick={onToggleSound} className={`pressable-button pressable-button-large ${pressClass("sound")} bg-transparent border-0 p-0 origin-center`} title={soundEnabled ? "Sound ON" : "Sound OFF"}>
          ...
          <button type="button" onPointerDown={() => markButtonPressed("vibration")} onClick={onToggleVibration} className={`pressable-button pressable-button-large ${pressClass("vibration")} bg-transparent border-0 p-0 origin-center`} title={vibrationEnabled ? "Vibration ON" : "Vibration OFF"}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button type="button" onPointerDown={() => markButtonPressed("resume")} onClick={onResume} className={`pressable-button pressable-button-large ${pressClass("resume")} bg-transparent border-0 p-0 origin-center`}>
            <img src={resumeButton} alt="Resume" className="w-full h-auto" draggable={false} />
          </button>
          <button type="button" onPointerDown={() => markButtonPressed("restart")} onClick={onRestart} className={`pressable-button pressable-button-large ${pressClass("restart")} bg-transparent border-0 p-0 origin-center`}>
