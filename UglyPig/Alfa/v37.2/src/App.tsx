...
const getDifficultyForLevel = (level: number) =>
  DIFFICULTY_LEVELS[Math.min(DIFFICULTY_LEVELS.length - 1, Math.max(0, level - 1))];

const getDifficultyReferenceScore = (topScore: number) => Math.max(MIN_DIFFICULTY_REFERENCE_SCORE, Math.max(0, topScore));

const getDifficultyStep = (referenceScore: number) =>
  Math.max(5, Math.ceil(referenceScore / DIFFICULTY_LEVELS.length));

const getRunDifficultySettings = (isFirstInstallRun: boolean, topScore: number) => {
  const referenceScore = isFirstInstallRun ? MIN_DIFFICULTY_REFERENCE_SCORE : getDifficultyReferenceScore(topScore);
...
const getDifficultyLevelForScore = (score: number, step: number) =>
  Math.min(DIFFICULTY_LEVELS.length, Math.floor(score / step) + 1);
...
export default function App() {
  ...
  const resetRun = useCallback(() => {
    ...
    const difficultySettings = getRunDifficultySettings(state.isFirstInstallRun, state.topScore);
  ...
  useEffect(() => {
    try {
      ...
      const difficultySettings = getRunDifficultySettings(isFirstInstallRun, gameStateRef.current.topScore);
  ...
  useEffect(() => {
    ...
    const gameLoop = (timestamp: number) => {
      ...
      if (hasStarted && !state.isGameOver && !isPaused && countdown === null && !isLandscape) {
        ...
        if (state.gameTime - state.lastSpawnTime > currentInterval && hasVerticalSpace) {
        ...
        state.fences.forEach((fence) => {
          ...
          if (!fence.passed && fence.y > PIG_Y + PIG_SIZE) {
            ...
            const nextDifficultyLevel = getDifficultyLevelForScore(state.score, state.difficultyStep);
  ...
  return (
    <div className="w-screen h-screen bg-sky-300 overflow-hidden">
      ...
      >
        ...
        {hasStarted && (
          <div className="absolute top-2 left-2 z-30 rounded-lg bg-black/55 px-2 py-2 text-[10px] font-black text-white shadow-lg pointer-events-none">
            NIVEL {renderState.difficultyLevel} | REF {renderState.difficultyReferenceScore} | STEP {renderState.difficultyStep}
          </div>
        )}
