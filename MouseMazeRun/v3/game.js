...
const CAMERA_VISIBLE_RADIUS = 3;
const CAMERA_ZOOM = (SIZE / (CAMERA_VISIBLE_RADIUS * 2 + 1)) * 1.54;
const CAMERA_VERTICAL_CENTER = 0.5;
const MAX_ATTEMPTS = 7;
const ATTEMPT_RECOVERY_MS = 30 * 60 * 1000;
const ATTEMPT_STORAGE_KEY = "mouseMazeAttemptStateV3Vision1";
const CAMPAIGN_STORAGE_KEY = "mouseMazeCampaignStateV3Vision1";
...
const MOUSE_SNIFF_DURATION_MS = 1050;
const LEVEL_INTRO_DURATION_MS = 2400;
const LEVEL_INTRO_RETRY_DURATION_MS = 1500;
const LEVEL_ONE_TIME_LIMIT_MS = 60 * 1000;
const TIME_LIMIT_STEP_MS = 5 * 1000;
const TIMER_SLOW_WARNING_MS = 6 * 1000;
const TIMER_FAST_WARNING_MS = 3 * 1000;
...
const ECONOMY_STORAGE_KEY = "mouseMazeEconomyStateV3Vision1";
const POWER_CONFIGS = {
  crystal: {
    name: "Crystal Vision",
    price: 10,
...
const timerStatEl = document.querySelector("#timerStat");
...
let moveLimit = 0;
let movesTaken = 0;
let timeLimitMs = LEVEL_ONE_TIME_LIMIT_MS;
let timeRemainingMs = timeLimitMs;
let timerStarted = false;
let timerRunning = false;
let timerExpiredPending = false;
...
let mouseSniffAnimation = null;
let mouseSniffFrame = null;
let levelIntro = null;
...
function levelTimeLimit(levelNumber = level) {
  return LEVEL_ONE_TIME_LIMIT_MS + Math.max(0, levelNumber - 1) * TIME_LIMIT_STEP_MS;
}

function updateTimerUI() {
  const ratio = timeLimitMs > 0 ? clamp(timeRemainingMs / timeLimitMs, 0, 1) : 0;
  timerFillEl.style.transform = `scaleX(${ratio})`;

  const slowWarning =
    timerRunning && timeRemainingMs > TIMER_FAST_WARNING_MS && timeRemainingMs <= TIMER_SLOW_WARNING_MS;
  const fastWarning =
    timerRunning && timeRemainingMs > 0 && timeRemainingMs <= TIMER_FAST_WARNING_MS;
  timerStatEl.classList.toggle("timer-warning-slow", slowWarning);
  timerStatEl.classList.toggle("timer-warning-fast", fastWarning);

  const seconds = Math.max(0, Math.ceil(timeRemainingMs / 1000));
  timerStatEl.setAttribute(
    "aria-label",
    timerStarted
      ? `${seconds} second${seconds === 1 ? "" : "s"} remaining`
      : `Time ready. ${seconds} seconds available`,
  );
}

function timerShouldPause() {
  return Boolean(
    document.hidden ||
      !powerShopEl.hidden ||
      levelIntro ||
...
function animateLevelTimer(timestamp) {
  if (!timerRunning) return;
  if (timerLastTimestamp === null) timerLastTimestamp = timestamp;
  const elapsed = Math.max(0, timestamp - timerLastTimestamp);
  ...
  timerFrame = requestAnimationFrame(animateLevelTimer);
}

function startLevelTimer() {
  if (timerStarted || gameOver || campaignComplete) return;
  timerStarted = true;
  timerRunning = true;
  timerLastTimestamp = null;
  updateTimerUI();
...
function updatePowerFinishWarnings() {
  const showSuggestions =
    ...
    timerStarted &&
    timeRemainingMs > 0 &&
    timeRemainingMs <= TIMER_FAST_WARNING_MS;
...
function updatePowerUI() {
  ...
  mazeEl.setAttribute(
    ...
    levelIntro
      ...
      : mouseSniffAnimation
        ? "Mouse sniffing the direction of the cheese"
      : powerTransform
...
function activateCrystalPower() {
  ...
  crystalCandidateIncomplete = false;
  crystalCandidatePath = path.slice(1);
...
function activateTornadoPower() {
  ...
  if (!tornadoCandidate) {
    ...
    setMessage("No safe maze variation is available from this square.", true);
    return;
  }
...
function drawMaze() {
  ...
  const geometry = boardGeometry(bounds.width, bounds.height);
  const camera = mazeCameraState(geometry, bounds.width, bounds.height);
  mazeLayout = { corners: geometry.inner, width: bounds.width, camera };

  mazeContext.save();
  mazeContext.translate(camera.center.x, camera.center.y);
  mazeContext.scale(camera.zoom, camera.zoom);
  mazeContext.translate(-camera.focus.x, -camera.focus.y);
  drawMazeBoard(mazeContext, bounds.width, bounds.height, geometry);
  mazeContext.restore();

  mazeLayout = {
    corners: transformCameraCorners(geometry.inner, camera),
...
function crystalCameraPosition() {
  if (!crystalRevealing || !crystalRevealPath.length || crystalVisibleCount <= 0) return null;
  const activeIndex = Math.min(crystalRevealPath.length - 1, crystalVisibleCount - 1);
  const current = crystalRevealPath[activeIndex];
  const previous = crystalRevealPath[Math.max(0, activeIndex - 1)];
  let amount = crystalRevealPhase === "reveal" ? smoothStep(crystalRevealStepProgress) : 1;

  if (crystalRevealPhase === "fade") {
    const returnAmount = smoothStep(1 - crystalRevealOpacity);
    return {
      row: current.row + (mouse.row - current.row) * returnAmount,
      col: current.col + (mouse.col - current.col) * returnAmount,
    };
  }

  return {
    row: previous.row + (current.row - previous.row) * amount,
    col: previous.col + (current.col - previous.col) * amount,
  };
}

function mazeCameraState(geometry, width, height) {
  const cameraPosition = crystalCameraPosition() ?? currentMouseVisualState();
  let focus = project(
    geometry.inner,
    (cameraPosition.col + 0.5) / SIZE,
    (cameraPosition.row + 0.5) / SIZE,
  );
  if (tornadoWallAnimation) {
    focus = tornadoVortexCenterAt(
      tornadoWallAnimation.progress,
      focus,
      project(geometry.inner, 0.5, 0.5),
    );
  }
  return {
    center: { x: width * 0.5, y: height * CAMERA_VERTICAL_CENTER },
    focus,
    zoom: CAMERA_ZOOM,
  };
}

function transformCameraPoint(point, camera) {
  return {
    x: camera.center.x + (point.x - camera.focus.x) * camera.zoom,
    y: camera.center.y + (point.y - camera.focus.y) * camera.zoom,
  };
}

function transformCameraCorners(corners, camera) {
  return {
    tl: transformCameraPoint(corners.tl, camera),
    tr: transformCameraPoint(corners.tr, camera),
    br: transformCameraPoint(corners.br, camera),
    bl: transformCameraPoint(corners.bl, camera),
  };
}

function drawMazeBoard(ctx, width, height, geometry = boardGeometry(width, height)) {
...
function finishCrystalReveal(token) {
  ...
  startPowerTransformation("crystal", "out", () => {
    ...
    setMessage(
      incomplete
        ? "The vision faded before reaching the cheese. Another power may open the way."
...
function generalCheeseDirection(from) {
  if (!exit || !from) return null;
  const rowDelta = exit.row - from.row;
  const colDelta = exit.col - from.col;
  if (!rowDelta && !colDelta) return null;

  const sectorAngle = Math.round(Math.atan2(rowDelta, colDelta) / (Math.PI / 4)) * (Math.PI / 4);
  return {
    row: Math.round(Math.sin(sectorAngle)),
    col: Math.round(Math.cos(sectorAngle)),
  };
}

function drawScentDirection(ctx, corners, center, cellWidth, progress, position) {
  const direction = generalCheeseDirection(position);
  if (!direction || progress <= 0) return;

  const directionPoint = project(
    corners,
    (position.col + 0.5 + direction.col) / SIZE,
    (position.row + 0.5 + direction.row) / SIZE,
  );
  const rawX = directionPoint.x - center.x;
  const rawY = directionPoint.y - center.y;
  const rawLength = Math.hypot(rawX, rawY) || 1;
  const axis = { x: rawX / rawLength, y: rawY / rawLength };
  const normal = { x: -axis.y, y: axis.x };
  const reveal = Math.sin(Math.PI * clamp((progress - 0.04) / 0.96, 0, 1));
  const drift = smoothStep(clamp(progress / 0.78, 0, 1));
  const startDistance = cellWidth * 0.4;
  const trailLength = cellWidth * (1.2 + drift * 0.42);

  ctx.save();
  ctx.globalAlpha = reveal;
  ctx.lineCap = "round";
  ctx.lineWidth = Math.max(1.4, cellWidth * 0.052);
  ctx.strokeStyle = "rgba(220, 157, 70, 0.76)";
  ctx.shadowColor = "rgba(238, 185, 86, 0.8)";
  ctx.shadowBlur = cellWidth * 0.16;

  for (let index = 0; index < 3; index += 1) {
    const offset = startDistance + trailLength * (0.18 + index * 0.28);
    const wave = Math.sin(progress * Math.PI * 5 + index * 1.9) * cellWidth * 0.1;
    const start = {
      x: center.x + axis.x * (offset - cellWidth * 0.2) + normal.x * wave,
      y: center.y + axis.y * (offset - cellWidth * 0.2) + normal.y * wave,
    };
    const end = {
      x: center.x + axis.x * (offset + cellWidth * 0.16) - normal.x * wave * 0.48,
      y: center.y + axis.y * (offset + cellWidth * 0.16) - normal.y * wave * 0.48,
    };
    ctx.beginPath();
    ctx.moveTo(start.x, start.y);
    ctx.quadraticCurveTo(
      (start.x + end.x) / 2 + normal.x * cellWidth * 0.12,
      (start.y + end.y) / 2 + normal.y * cellWidth * 0.12,
      end.x,
      end.y,
    );
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.fillStyle = "rgba(249, 213, 137, 0.9)";
    ctx.arc(end.x, end.y, cellWidth * (0.045 + index * 0.01), 0, Math.PI * 2);
    ctx.fill();
  }
...
function drawCharacters(ctx, corners) {
  ...
  const activeSniffProgress = levelIntro
    ? levelIntroStateAt(levelIntro.progress).sniffProgress
    : mouseSniffAnimation?.progress ?? 0;
  if (activeSniffProgress > 0) {
    drawScentDirection(ctx, corners, mouseCenter, mouseCellWidth, activeSniffProgress, visualMouse);
  }
  ...
  } else if (mouseSniffAnimation) {
    const sniffFrame = Math.min(3, Math.floor(mouseSniffAnimation.progress * 4));
    const sniffPulse = Math.sin(mouseSniffAnimation.progress * Math.PI * 4) * 0.01;
    if (mouseSniffSprite.complete && mouseSniffSprite.naturalWidth) {
      drawSpriteFrame(
        ctx,
        mouseSniffSprite,
        sniffFrame,
      ...
      drawNeutralMouse(ctx, mouseCenter, mouseCellWidth);
    }
...
function clearMouseSniffAnimation() {
  if (mouseSniffFrame !== null) cancelAnimationFrame(mouseSniffFrame);
  mouseSniffAnimation = null;
  mouseSniffFrame = null;
  mazeEl.setAttribute("aria-busy", "false");
}

function finishMouseSniffAnimation() {
  if (!mouseSniffAnimation) return;
  mouseSniffAnimation = null;
  mouseSniffFrame = null;
  mazeEl.setAttribute("aria-busy", "false");
  if (!gameOver && !campaignComplete) {
    setMovementControlsEnabled(true);
    setPowerControlsEnabled(true);
  }
  render();
...
function animateMouseSniff(timestamp) {
  if (!mouseSniffAnimation) return;
  if (mouseSniffAnimation.startedAt === null) mouseSniffAnimation.startedAt = timestamp;
  mouseSniffAnimation.progress = clamp(
    (timestamp - mouseSniffAnimation.startedAt) / MOUSE_SNIFF_DURATION_MS,
    0,
    1,
  );
  drawMaze();

  if (mouseSniffAnimation.progress >= 1) {
    finishMouseSniffAnimation();
    return;
  }
  mouseSniffFrame = requestAnimationFrame(animateMouseSniff);
}

function canStartMouseSniff() {
  return (
    !gameOver &&
    !campaignComplete &&
    !levelIntro &&
    ...
    !powerTransform &&
    !isPowerTargeting() &&
    Boolean(generalCheeseDirection(mouse))
  );
}

function startMouseSniffAnimation() {
  if (!canStartMouseSniff()) return false;
  mouseSniffAnimation = { progress: 0, startedAt: null };
  setMovementControlsEnabled(false);
  setPowerControlsEnabled(false);
  mazeEl.setAttribute("aria-busy", "true");
  requestMazeDraw();
...
function finishLevelIntroAnimation() {
  ...
  setMovementControlsEnabled(true);
  setPowerControlsEnabled(true);
  setMessage("Find the cheese. Time starts with your first move.");
  render();
  saveCampaignState();
}
...
function finishMouseMotion() {
  ...
  if (timerExpiredPending) {
    timerExpiredPending = false;
    startMouseDefeatAnimation();
    return;
  }

  setMessage("Keep exploring. The cheese is somewhere ahead.");
  saveCampaignState();
...
function animateMouseDefeat(timestamp) {
  ...
  if (mouseDefeatProgress >= 1) {
    ...
    mazeEl.setAttribute("aria-busy", "false");
    requestMazeDraw();
    loseLevel("Time ran out. Retry with what you discovered.");
    return;
  }
...
function startMouseDefeatAnimation() {
  ...
  setPowerControlsEnabled(false);
  updateTimerUI();
  setMessage("Time is up. The mouse is getting sleepy...");
  mazeEl.setAttribute("aria-busy", "true");
  requestMazeDraw();
...
function loseLevel(message = "Time is up. Retry with what you discovered.") {
  finishLevelTimer();
  gameOver = true;
  retryCostsAttempt = true;
  clearCheeseEatingAnimation();
...
function calculateStars() {
  const remainingRatio = timeLimitMs > 0 ? timeRemainingMs / timeLimitMs : 0;
...
mazeEl.addEventListener("click", (event) => {
  if (!canStartMouseSniff() || !mazeLayout) return;
  const bounds = mazeEl.getBoundingClientRect();
  const point = { x: event.clientX - bounds.left, y: event.clientY - bounds.top };
  if (characterTargetAtPoint(point)) startMouseSniffAnimation();
});
