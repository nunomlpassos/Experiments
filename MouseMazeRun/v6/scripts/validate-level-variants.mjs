...
const expectedCodes = [
  "T", "Mf", "F", "N", "F", "N", "D", "D", "Md", "Md", "F", "E", "Md", "E",
  "D", "N", "Md", "Md", "F", "D", "N", "D", "Md", "E", "Md", "E", "Md", "I",
  "N", "D", "D", "N", "Md", "N", "Md", "E", "Md", "E", "E", "D", "E", "I",
  "D", "Md", "E", "E", "I", "Md", "D", "Md", "E", "I", "D", "Md", "E", "I",
];
if (LEVEL_CONFIGS.length !== 56 || WORLD_LEVELS.length !== 56) {
  throw new Error("The campaign must contain four sections of 14 levels");
}
...
if (
  ...
  LEVEL_CONFIGS[19]?.pieCount !== 1 ||
  LEVEL_CONFIGS[21]?.hingedWallCount !== 1 ||
  !LEVEL_CONFIGS[24]?.fishingIntro ||
  LEVEL_CONFIGS[23]?.pieCount !== 1 ||
  !LEVEL_CONFIGS[26]?.catPawIntro ||
  ...
  POWER_UNLOCK_LEVELS.fishing !== 25 ||
  POWER_UNLOCK_LEVELS.rocket !== 29
) {
  throw new Error("The second section introductions or power unlock levels are misplaced");
}
if (
  !LEVEL_CONFIGS[28]?.rocketIntro ||
  LEVEL_CONFIGS[29]?.difficultyCode !== "D" ||
  LEVEL_CONFIGS[30]?.tunnelCount !== 3 ||
  !LEVEL_CONFIGS[30]?.tunnelIntro ||
  !LEVEL_CONFIGS[33]?.shape ||
  LEVEL_CONFIGS[36]?.difficultyCode !== "Md" ||
  !LEVEL_CONFIGS[36]?.crowIntro ||
  LEVEL_CONFIGS[37]?.difficultyCode !== "E" ||
  LEVEL_CONFIGS[37]?.tunnelCount !== 3 ||
  LEVEL_CONFIGS[39]?.rotatingCircuitCount !== 1 ||
  !LEVEL_CONFIGS[39]?.rotatingTilesIntro ||
  adversitiesForVariant(LEVEL_CONFIGS[41], 0).length !== 2
) {
  throw new Error("The third-section difficulty curve or mechanic introductions are misplaced");
}
if (
  LEVEL_CONFIGS[42]?.cloudMode !== "intro" ||
  LEVEL_CONFIGS[45]?.waterMode !== "intro" ||
  LEVEL_CONFIGS[46]?.waterMode !== "advanced" ||
  LEVEL_CONFIGS[55]?.waterMode !== "advanced" ||
  LEVEL_CONFIGS[54]?.rockCount !== 3
) {
  throw new Error("The fourth-section mechanic curve is misplaced");
}
...
level = 1;
for (const [variantIndex, tunnelVariant] of globalThis.__MOUSE_MAZE_PREGENERATED_LEVELS__[30].entries()) {
  ...
  if (
    ...
    throw new Error("Level 31 variant " + (variantIndex + 1) + " has invalid tunnel placement");
  }
}
for (const [variantIndex, rotatingVariant] of globalThis.__MOUSE_MAZE_PREGENERATED_LEVELS__[39].entries()) {
  ...
  const invalidCell = (circuit?.cells ?? []).some(
    (cell) =>
      ...
      cell.row <= 0 ||
      cell.row >= (LEVEL_CONFIGS[39]?.rows ?? 9) - 1 ||
      cell.col <= 0 ||
      cell.col >= (LEVEL_CONFIGS[39]?.cols ?? 9) - 1,
  );
  if (
    ...
    throw new Error("Level 40 variant " + (variantIndex + 1) + " has an invalid rotating circuit");
  }
}
...
if (isPowerUnlocked("fishing")) {
  throw new Error("The Fishing Rod must stay locked until its level-25 tutorial begins");
}
powerIntroTutorialUnlockedForRun.add("fishing");
if (!isPowerUnlocked("fishing")) {
  throw new Error("The Fishing Rod must unlock when its level-25 tutorial begins");
}
powerIntroTutorialUnlockedForRun = new Set();
level = 29;
if (isPowerUnlocked("rocket")) {
  throw new Error("The Rocket must stay locked until its level-29 tutorial begins");
}
powerIntroTutorialUnlockedForRun.add("rocket");
if (!isPowerUnlocked("rocket")) {
  throw new Error("The Rocket must unlock when its level-29 tutorial begins");
}
globalThis.__unlockAllPowersForValidation = true;
level = 1;
if (
...
level = 37;
setGridDimensions(LEVEL_CONFIGS[36]);
const crowVariant = buildLevelVariant(LEVEL_CONFIGS[36], 0);
...
if (escapedCrowPosition) {
  activeLevelConfig = { ...LEVEL_CONFIGS[36], crowMode: true };
...
if (
  LEVEL_CONFIGS[41]?.difficultyCode !== "I" ||
  LEVEL_CONFIGS[41]?.tunnelCount !== 3 ||
  LEVEL_CONFIGS[41]?.rotatingCircuitCount !== 1 ||
  Array.from({ length: VARIANTS_PER_LEVEL }, (_, variantIndex) =>
    adversitiesForVariant(LEVEL_CONFIGS[41], variantIndex).length
  ).some((count) => count !== 2)
) {
  throw new Error("Level 42 must combine tunnels, rotating floors, and two adversities");
}
level = 15;
...
if (!LEVEL_CONFIGS[12].tornadoIntro || LEVEL_CONFIGS[21].hingedWallCount !== 1) {
...
for (const introLevel of [7, 13, 17, 25, 29]) {
  levelVariantStates[introLevel - 1] = { current: -1, remaining: [] };
  if (takeNextVariant(introLevel - 1) !== 0) {
    throw new Error(
      "Power introduction level " + introLevel + " must begin with its curated variant",
    );
  }
}
...
if (!LEVEL_CONFIGS[28]?.rocketIntro || POWER_UNLOCK_LEVELS.rocket !== 29) {
  throw new Error("Level 29 must introduce and unlock the rocket");
}
milkBottles = [{
  id: "rocket-milk",
  row: 0,
...
level = 46;
activeLevelConfig = LEVEL_CONFIGS[45];
